...
ConstGenJetIntTagAssociation::~ConstGenJetIntTagAssociation(){
  ...
  const fcc::ConstGenJet ConstGenJetIntTagAssociation::Jet() const { if (m_obj->m_Jet == nullptr) {
 return fcc::ConstGenJet(nullptr);}
 return fcc::ConstGenJet(*(m_obj->m_Jet));}
  const fcc::ConstIntTag ConstGenJetIntTagAssociation::Tag() const { if (m_obj->m_Tag == nullptr) {
