...
ParticleMCParticleAssociation::operator ConstParticleMCParticleAssociation() const {return ConstParticleMCParticleAssociation(m_obj);}
  ...
  const ConstParticle ParticleMCParticleAssociation::Rec() const { if (m_obj->m_Rec == nullptr) {
 return ConstParticle(nullptr);}
 return ConstParticle(*(m_obj->m_Rec));}
  const ConstMCParticle ParticleMCParticleAssociation::Sim() const { if (m_obj->m_Sim == nullptr) {
 return ConstMCParticle(nullptr);}
 return ConstMCParticle(*(m_obj->m_Sim));}

void ParticleMCParticleAssociation::Rec(ConstParticle value) { if (m_obj->m_Rec != nullptr) delete m_obj->m_Rec; m_obj->m_Rec = new ConstParticle(value); }
void ParticleMCParticleAssociation::Sim(ConstMCParticle value) { if (m_obj->m_Sim != nullptr) delete m_obj->m_Sim; m_obj->m_Sim = new ConstMCParticle(value); }
