...
ParticleMCParticleAssociation::operator ConstParticleMCParticleAssociation() const {return ConstParticleMCParticleAssociation(m_obj);}
  ...
  const fcc::ConstParticle ParticleMCParticleAssociation::Rec() const { if (m_obj->m_Rec == nullptr) {
 return fcc::ConstParticle(nullptr);}
 return fcc::ConstParticle(*(m_obj->m_Rec));}
  const fcc::ConstMCParticle ParticleMCParticleAssociation::Sim() const { if (m_obj->m_Sim == nullptr) {
 return fcc::ConstMCParticle(nullptr);}
 return fcc::ConstMCParticle(*(m_obj->m_Sim));}

void ParticleMCParticleAssociation::Rec(fcc::ConstParticle value) { if (m_obj->m_Rec != nullptr) delete m_obj->m_Rec; m_obj->m_Rec = new ConstParticle(value); }
void ParticleMCParticleAssociation::Sim(fcc::ConstMCParticle value) { if (m_obj->m_Sim != nullptr) delete m_obj->m_Sim; m_obj->m_Sim = new ConstMCParticle(value); }
