...
ParticleTrackAssociationCollection::ParticleTrackAssociationCollection() : m_isValid(false), m_collectionID(0), m_entries() , m_rel_Particle(new std::vector<fcc::ConstParticle>()), m_rel_Track(new std::vector<fcc::ConstTrack>()),m_data(new ParticleTrackAssociationDataContainer() ) {
    m_refCollections.push_back(new std::vector<podio::ObjectID>());
  m_refCollections.push_back(new std::vector<podio::ObjectID>());

}

ParticleTrackAssociationCollection::~ParticleTrackAssociationCollection() {
  clear();
...
void ParticleTrackAssociationCollection::prepareForWrite(){
  ...
  for (auto& obj : m_entries) {
    if (obj->m_Track != nullptr) {
      m_refCollections[1]->emplace_back(obj->m_Track->getObjectID());
    } else {
