...
SimCaloClusterHitAssociationCollection::SimCaloClusterHitAssociationCollection() : m_isValid(false), m_collectionID(0), m_entries() , m_rel_Cluster(new std::vector<fcc::ConstSimCaloCluster>()), m_rel_Hit(new std::vector<fcc::ConstSimCaloHit>()),m_data(new SimCaloClusterHitAssociationDataContainer() ) {
    m_refCollections.push_back(new std::vector<podio::ObjectID>());
  m_refCollections.push_back(new std::vector<podio::ObjectID>());

}

SimCaloClusterHitAssociationCollection::~SimCaloClusterHitAssociationCollection() {
  clear();
...
void SimCaloClusterHitAssociationCollection::prepareForWrite(){
  ...
  for (auto& obj : m_entries) {
    if (obj->m_Hit != nullptr) {
      m_refCollections[1]->emplace_back(obj->m_Hit->getObjectID());
    } else {
