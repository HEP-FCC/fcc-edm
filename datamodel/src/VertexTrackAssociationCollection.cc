...
VertexTrackAssociationCollection::VertexTrackAssociationCollection() : m_isValid(false), m_collectionID(0), m_entries() , m_rel_Track(new std::vector<fcc::ConstTrack>()), m_rel_Vertex(new std::vector<fcc::ConstVertex>()),m_data(new VertexTrackAssociationDataContainer() ) {
    m_refCollections.push_back(new std::vector<podio::ObjectID>());
  m_refCollections.push_back(new std::vector<podio::ObjectID>());

}

VertexTrackAssociationCollection::~VertexTrackAssociationCollection() {
  clear();
  if (m_data != nullptr) delete m_data;
    for (auto& pointer : m_refCollections) { if (pointer != nullptr) delete pointer; }
  if (m_rel_Track != nullptr) { delete m_rel_Track; }
  if (m_rel_Vertex != nullptr) { delete m_rel_Vertex; }
