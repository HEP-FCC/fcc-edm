...
ConstJetParticleAssociation::~ConstJetParticleAssociation(){
  ...
  const fcc::ConstJet ConstJetParticleAssociation::Jet() const {
    if (m_obj->m_Jet == nullptr) {
      return fcc::ConstJet(nullptr);
    }
    return fcc::ConstJet(*(m_obj->m_Jet));} const fcc::ConstParticle ConstJetParticleAssociation::Particle() const {
