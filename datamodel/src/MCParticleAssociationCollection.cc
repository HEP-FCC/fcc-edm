...
MCParticleAssociationCollection::MCParticleAssociationCollection() : m_isValid(false), m_collectionID(0), m_entries() , m_rel_Mother(new std::vector<fcc::ConstMCParticle>()), m_rel_Daughter(new std::vector<fcc::ConstMCParticle>()),m_data(new MCParticleAssociationDataContainer() ) {
    m_refCollections.push_back(new std::vector<podio::ObjectID>());
  m_refCollections.push_back(new std::vector<podio::ObjectID>());

}

MCParticleAssociationCollection::~MCParticleAssociationCollection() {
  clear();
  if (m_data != nullptr) delete m_data;
    for (auto& pointer : m_refCollections) { if (pointer != nullptr) delete pointer; }
  if (m_rel_Mother != nullptr) { delete m_rel_Mother; }
  if (m_rel_Daughter != nullptr) { delete m_rel_Daughter; }
...
void MCParticleAssociationCollection::prepareForWrite(){
  ...
  for (auto& obj : m_entries) {
    if (obj->m_Mother != nullptr) {
      m_refCollections[0]->emplace_back(obj->m_Mother->getObjectID());
    } else {
  ...
  for (auto& obj : m_entries) {
    if (obj->m_Daughter != nullptr) {
      m_refCollections[1]->emplace_back(obj->m_Daughter->getObjectID());
    } else {
