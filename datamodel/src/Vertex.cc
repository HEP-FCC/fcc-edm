...
Vertex::Vertex(float Chi2,unsigned Ndf,Point Position,unsigned Bits) : m_obj(new VertexObj()){
 m_obj->acquire();
...
Vertex::operator ConstVertex() const {return ConstVertex(m_obj);}
  ...
  const Point& Vertex::Position() const { return m_obj->data.Position; }
...
void Vertex::Ndf(unsigned value){ m_obj->data.Ndf = value;}
  Point& Vertex::Position() { return m_obj->data.Position; }
void Vertex::Position(class Point value){ m_obj->data.Position = value;}
