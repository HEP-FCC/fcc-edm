...
//  MCParticleAssociationCollection(MCParticleAssociationVector* data, int collectionID);
  ...
  void setID(unsigned ID){
    ...
    std::for_each(m_entries.begin(),m_entries.end(),
                 [ID](MCParticleAssociationObj* obj){obj->id = {obj->id.index,static_cast<int>(ID)}; }
    );
  };
...
private:
  ...
  std::vector<fcc::ConstMCParticle>* m_rel_Mother; ///< Relation buffer for read / write
  std::vector<fcc::ConstMCParticle>* m_rel_Daughter; ///< Relation buffer for read / write
