...
//  TrackClusterHitsAssociationCollection(TrackClusterHitsAssociationVector* data, int collectionID);
  ...
  void setID(unsigned ID){
    ...
    std::for_each(m_entries.begin(),m_entries.end(),
                 [ID](TrackClusterHitsAssociationObj* obj){obj->id = {obj->id.index,static_cast<int>(ID)}; }
    );
  };
...
private:
  ...
  std::vector<fcc::ConstTrackCluster>* m_rel_Cluster; ///< Relation buffer for read / write
  std::vector<fcc::ConstTrackHit>* m_rel_Hit; ///< Relation buffer for read / write
